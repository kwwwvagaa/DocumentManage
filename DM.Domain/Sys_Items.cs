...
{
    /// <summary>	
    /// 数据表实体类：选项主表 (Sys_Items)
    /// </summary>
    [Serializable()]
    public partial class Sys_Items:BaseModel
    {
	   ...
	   { 
            get
            {
                BeforeGetProperty("F_IsTree", _F_IsTree);
                return _F_IsTree;
            }
            set
            {
                if(!BeforeSetProperty("F_IsTree", _F_IsTree))
                    return;
                _F_IsTree=value;
                AfterSetProperty("F_IsTree", _F_IsTree);
             }
		}
